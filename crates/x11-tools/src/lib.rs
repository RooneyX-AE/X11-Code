...
impl ToolContext {
    pub async fn path(&self, p: &str, write: bool) -> Result<PathBuf> {
        let rel = Path::new(p);
        if rel.is_absolute() || rel.components().any(|c| matches!(c, Component::ParentDir)) { anyhow::bail!("path escapes workspace") }
...
fn truncate(s: impl Into<String>) -> String { let mut s=s.into(); if s.len()<=MAX_OUTPUT{return s;} s.truncate(MAX_OUTPUT); s.push_str("\n...[output truncated by X11 Code]..."); s }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ToolKind { ReadOnly, FilesystemWrite, Shell, GitWrite, Network }
#[async_trait]
pub trait Tool: Send + Sync { fn name(&self)->&'static str; fn description(&self)->&'static str; fn kind(&self)->ToolKind; fn input_schema(&self)->Value; async fn execute(&self,ctx:&ToolContext,input:Value)->Result<String>; }
...
impl ToolRegistry {
    ...
    pub fn definitions(&self)->Vec<Value>{let mut v=self.tools.values().map(|t|json!({"name":t.name(),"description":t.description(),"input_schema":t.input_schema()})).collect::<Vec<_>>();v.sort_by(|a,b|a["name"].as_str().cmp(&b["name"].as_str()));v}
