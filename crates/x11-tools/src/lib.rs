...
impl ToolContext {
    pub async fn root(&self) -> Result<PathBuf> {
        Ok(tokio::fs::canonicalize(&self.workspace).await?)
    }
...
#[async_trait] impl Tool for ReadFile {
    fn name(&self)->&'static str{"read_file"} fn description(&self)->&'static str{"Read a UTF-8 file inside the workspace."} fn kind(&self)->ToolKind{ToolKind::ReadOnly}
...
#[async_trait] impl Tool for WriteFile {
    fn name(&self)->&'static str{"write_file"} fn description(&self)->&'static str{"Write or replace a UTF-8 file inside the workspace."} fn kind(&self)->ToolKind{ToolKind::FilesystemWrite}
...
#[async_trait] impl Tool for EditFile {
    fn name(&self)->&'static str{"edit_file"} fn description(&self)->&'static str{"Replace exactly one text occurrence in a workspace file."} fn kind(&self)->ToolKind{ToolKind::FilesystemWrite}
...
#[async_trait] impl Tool for ListFiles {
    fn name(&self)->&'static str{"list_files"} fn description(&self)->&'static str{"List files and directories below a workspace path."} fn kind(&self)->ToolKind{ToolKind::ReadOnly}
    fn input_schema(&self)->Value{json!({"type":"object","properties":{"path":{"type":"string"},"depth":{"type":"integer","minimum":0,"maximum":8}},"required":[],"additionalProperties":false})}
...
#[async_trait] impl Tool for Shell {
    fn name(&self)->&'static str{"shell"} fn description(&self)->&'static str{"Run a bounded shell command in the workspace."} fn kind(&self)->ToolKind{ToolKind::Shell}
    fn input_schema(&self)->Value{json!({"type":"object","properties":{"command":{"type":"string"},"timeout_ms":{"type":"integer","minimum":100,"maximum":120000}},"required":["command"],"additionalProperties":false})}
...
#[async_trait] impl Tool for Search {
    fn name(&self)->&'static str{"search"} fn description(&self)->&'static str{"Search text recursively, using ripgrep when available."} fn kind(&self)->ToolKind{ToolKind::ReadOnly}
    fn input_schema(&self)->Value{json!({"type":"object","properties":{"pattern":{"type":"string"},"path":{"type":"string"}},"required":["pattern"],"additionalProperties":false})}
...
#[async_trait] impl Tool for Git {
    fn name(&self)->&'static str{"git"} fn description(&self)->&'static str{"Run git with an explicit argument array."} fn kind(&self)->ToolKind{ToolKind::GitWrite}
    fn input_schema(&self)->Value{json!({"type":"object","properties":{"args":{"type":"array","items":{"type":"string"}}},"required":["args"],"additionalProperties":false})}
...
#[async_trait] impl Tool for GitStatus {
    fn name(&self)->&'static str{"git_status"} fn description(&self)->&'static str{"Inspect repository status without modifying it."} fn kind(&self)->ToolKind{ToolKind::ReadOnly}
    fn input_schema(&self)->Value{json!({"type":"object","properties":{},"required":[],"additionalProperties":false})}
    async fn execute(&self,c:&ToolContext,_:Value)->Result<String>{let o=Command::new("git").args(["status","--short","--branch"]).current_dir(&c.workspace).output().await?;Ok(truncate(format!("exit={}\n{}",o.status.code().unwrap_or(-1),String::from_utf8_lossy(&o.stdout))))}
}

pub struct GitDiff;
#[async_trait] impl Tool for GitDiff {
    fn name(&self)->&'static str{"git_diff"} fn description(&self)->&'static str{"Inspect the current git diff without modifying the repository."} fn kind(&self)->ToolKind{ToolKind::ReadOnly}
    fn input_schema(&self)->Value{json!({"type":"object","properties":{"cached":{"type":"boolean"}},"required":[],"additionalProperties":false})}
