    ...
    pub async fn path(&self, p: &str, write: bool) -> Result<PathBuf> {
        ...
        if write {
            if let Some(parent) = candidate.parent() {
                let canon_parent = if tokio::fs::try_exists(parent).await? { tokio::fs::canonicalize(parent).await? } else { parent.to_path_buf() };
...
#[cfg(test)]
mod tests{use super::*;#[test]fn registry_contains_core_tools(){let r=ToolRegistry::builtins();for n in ["read_file","write_file","edit_file","list_files","shell","search","git","git_status","git_diff"]{assert!(r.get(n).is_some());}}#[tokio::test]async fn parent_path_is_rejected(){let c=ToolContext{workspace:std::env::current_dir().unwrap()};assert!(c.path("../Cargo.toml",false).await.is_err());}}
