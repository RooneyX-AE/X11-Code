...
use crate::cosmic_state::CosmicPhase;
...
impl CosmicField {
    pub fn new(width: u16, height: u16) -> Self {
        ...
        Self { stars, frame: 0, phase: CosmicPhase::Idle }
    }
    ...
    pub fn overlay(&self, width: u16, height: u16) -> String {
        ...
        let density = match self.phase {
            CosmicPhase::Idle => 1,
            CosmicPhase::Running => 1,
            CosmicPhase::Conflict => 2,
            CosmicPhase::Resolving => 2,
            CosmicPhase::Verifying => 1,
            CosmicPhase::Completed => 1,
            CosmicPhase::Failed => 1,
        };
        ...
        if matches!(self.phase, CosmicPhase::Resolving | CosmicPhase::Verifying) && width > 4 && height > 2 {
            ...
            for (x, y) in [
                (cx as i32 + radius, cy as i32),
                (cx as i32 - radius, cy as i32),
                (cx as i32, cy as i32 + radius),
                (cx as i32, cy as i32 - radius),
            ] {
                if x >= 0 && y >= 0 && (x as u16) < width && (y as u16) < height {
                    grid[y as usize][x as usize] = if self.phase == CosmicPhase::Resolving { '◉' } else { '○' };
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn field_is_deterministic_for_same_size_and_phase() {
        let mut a = CosmicField::new(80, 20);
        let mut b = CosmicField::new(80, 20);
        a.set_phase(CosmicPhase::Running);
        b.set_phase(CosmicPhase::Running);
        assert_eq!(a.overlay(80, 20), b.overlay(80, 20));
    }

    #[test]
    fn phase_changes_visual_projection() {
        let mut field = CosmicField::new(40, 12);
        field.set_phase(CosmicPhase::Running);
        let running = field.overlay(40, 12);
        field.set_phase(CosmicPhase::Conflict);
        let conflict = field.overlay(40, 12);
        assert_ne!(running, conflict);
    }

    #[test]
    fn frame_changes_animation() {
        let mut field = CosmicField::new(40, 12);
        field.set_phase(CosmicPhase::Running);
        let before = field.overlay(40, 12);
        field.tick();
        assert_ne!(before, field.overlay(40, 12));
    }
}
