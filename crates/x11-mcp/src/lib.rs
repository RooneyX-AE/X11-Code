...
impl McpTool{
 pub fn qualified_name(&self,server:&str)->String{format!("mcp__{server}__{}",self.name)}
 pub fn is_valid_name(&self)->bool{!self.name.trim().is_empty()&&!self.name.contains("\n")}
}
...
impl McpClient{
 ...
 pub async fn list_tools(&mut self)->Result<Vec<McpTool>>{let r=self.request("tools/list",serde_json::json!({})).await?;let tools:Vec<McpTool>=serde_json::from_value(r["tools"].clone())?;if tools.iter().any(|t|!t.is_valid_name()){anyhow::bail!("MCP server returned an invalid tool name")}Ok(tools)}
 pub async fn call_tool(&mut self,name:&str,arguments:serde_json::Value)->Result<serde_json::Value>{if name.trim().is_empty(){anyhow::bail!("MCP tool name cannot be empty")}self.request("tools/call",serde_json::json!({"name":name,"arguments":arguments})).await}
