...
impl McpClient {
    pub async fn spawn(cfg: &McpServerConfig) -> Result<Self> {
        ...
        c.args(&cfg.args).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::inherit());
        for (k,v) in &cfg.env { c.env(k, v); }
        if let Some(cwd) = &cfg.cwd { c.current_dir(cwd); }
        let mut child = c.spawn().context("failed to start MCP server")?;
    ...
    async fn request(&mut self, method: &str, params: serde_json::Value) -> Result<serde_json::Value> {
        ...
        let body = serde_json::to_string(&RpcReq { jsonrpc:"2.0", id, method, params })?;
        ...
        loop {
            ...
            if n == 0 { anyhow::bail!("MCP server exited"); }
            if line.trim().is_empty() { continue; }
            let resp: RpcResp = serde_json::from_str(line.trim()).context("invalid MCP JSON-RPC response")?;
            if let Some(e) = resp.error { anyhow::bail!("MCP error: {e}"); }
            if let Some(r) = resp.result { return Ok(r); }
        }
    }

    pub async fn initialize(&mut self) -> Result<serde_json::Value> {
        self.request("initialize", serde_json::json!({"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"x11-code","version":"0.1.0"}})).await
    }
...
impl McpRegistry {
    pub fn register(&mut self, config: McpServerConfig) { self.servers.insert(config.name.clone(), config); }
    pub fn get(&self, name: &str) -> Option<&McpServerConfig> { self.servers.get(name) }
    pub fn iter(&self) -> impl Iterator<Item=&McpServerConfig> { self.servers.values() }
    pub fn enabled(&self) -> impl Iterator<Item=&McpServerConfig> { self.servers.values().filter(|s| s.enabled) }
    pub fn from_json(value: serde_json::Value) -> Result<Self> {
        let map = value.get("mcpServers").cloned().unwrap_or_else(|| serde_json::json!({}));
        let raw: BTreeMap<String, serde_json::Value> = serde_json::from_value(map)?;
        let mut registry = Self::default();
        for (name, v) in raw {
            let command = v.get("command").and_then(|x| x.as_str()).context("MCP server command missing")?.to_owned();
            registry.register(McpServerConfig { name, command, args: v.get("args").and_then(|x| serde_json::from_value(x.clone()).ok()).unwrap_or_default(), env: v.get("env").and_then(|x| serde_json::from_value(x.clone()).ok()).unwrap_or_default(), cwd: v.get("cwd").and_then(|x| x.as_str()).map(str::to_owned), enabled: v.get("enabled").and_then(|x| x.as_bool()).unwrap_or(true) });
        }
        Ok(registry)
    }
}
...
mod tests {
    use super::*;
    #[test]
    fn qualifies_mcp_tool_names() {
        let tool = McpTool { name:"create_issue".into(), description:None, input_schema:serde_json::json!({}) };
