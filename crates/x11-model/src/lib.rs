...
impl Message {
    ...
    pub fn assistant_with_tools(content: impl Into<String>, calls: &[ToolCall]) -> Self {
        Self { role: "assistant".into(), content: content.into(), tool_calls: calls.iter().map(WireToolCall::from_tool_call).collect(), tool_call_id: None }
    }
...
impl WireToolCall {
    fn from_tool_call(call: &ToolCall) -> Self {
        Self { id: call.id.clone(), kind: "function".into(), function: WireFunction { name: call.name.clone(), arguments: serde_json::to_string(&call.arguments).unwrap_or_else(|_| "{}".into()) } }
    }
}
...
impl ModelProvider for OpenAiCompatible {
    ...
    async fn complete(&self,r:CompletionRequest)->Result<CompletionResponse>{
        ...
        let response=loop{let response=self.client.post(&url).bearer_auth(&self.api_key).json(&body).send().await.context("model request failed")?;if response.status().is_success(){break response;}let status=response.status();if !Self::retryable(status)||attempt>=self.max_retries{let body=response.text().await.unwrap_or_default();let body=body.chars().take(4000).collect::<String>();anyhow::bail!("model API returned {}: {}",status,body);}attempt+=1;tokio::time::sleep(Duration::from_millis(250u64.saturating_mul(1u64<<attempt.min(4)))).await;};
        let envelope=response.json::<ChatEnvelope>().await.context("invalid model response JSON")?;let choice=envelope.choices.first().context("model response contained no choices")?;let mut calls=Vec::new();for c in &choice.message.tool_calls{let id=c.id.clone().unwrap_or_default();let name=c.function.name.clone().unwrap_or_default();if id.is_empty()||name.is_empty(){anyhow::bail!("model returned malformed tool call")}let raw=c.function.arguments.as_deref().unwrap_or("{}");let arguments=serde_json::from_str(raw).with_context(||format!("invalid JSON tool arguments for {name}"))?;calls.push(ToolCall{id,name,arguments});}let usage=envelope.usage.unwrap_or(UsageEnvelope{prompt_tokens:None,completion_tokens:None});Ok(CompletionResponse{text:choice.message.content.clone().unwrap_or_default(),tool_calls:calls,finish_reason:choice.finish_reason.clone(),usage:Usage{input_tokens:usage.prompt_tokens.unwrap_or(0).min(u32::MAX as u64) as u32,output_tokens:usage.completion_tokens.unwrap_or(0).min(u32::MAX as u64) as u32}})
    }
}

#[cfg(test)]
mod tests {use super::*;#[tokio::test]async fn mock_provider_is_deterministic(){let p=MockProvider;let r=p.complete(CompletionRequest{model:"m".into(),messages:vec![Message::user("hello")],tools:vec![],temperature:None,max_tokens:None}).await.unwrap();assert_eq!(r.text,"Mock provider received: hello");assert!(r.tool_calls.is_empty());}
#[test]fn assistant_tool_call_serializes_as_wire_format(){let call=ToolCall{id:"call_1".into(),name:"read_file".into(),arguments:serde_json::json!({"path":"src/main.rs"})};let msg=Message::assistant_with_tools("",&[call]);assert_eq!(msg.role,"assistant");assert_eq!(msg.tool_calls[0].kind,"function");assert_eq!(msg.tool_calls[0].function.arguments,"{\"path\":\"src/main.rs\"}");}
#[test]fn tool_result_preserves_call_id(){let msg=Message::tool("call_1","result");assert_eq!(msg.role,"tool");assert_eq!(msg.tool_call_id.as_deref(),Some("call_1"));}
