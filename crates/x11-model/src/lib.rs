...
impl ModelProvider for MockProvider {
    ...
    async fn complete(&self,request:CompletionRequest)->Result<CompletionResponse>{
        let goal=request.messages.iter().rev().find(|m|m.role=="user").map(|m|m.content.clone()).unwrap_or_default();
        Ok(CompletionResponse{text:format!("Mock provider received: {goal}"),finish_reason:Some("stop".into()),..Default::default()})
    }
}
...
impl ModelProvider for OpenAiCompatible {
    ...
    async fn complete(&self,r:CompletionRequest)->Result<CompletionResponse>{
        ...
        for c in &choice.message.tool_calls {
            ...
            if id.is_empty()||name.is_empty(){anyhow::bail!("model returned malformed tool call")}
            let raw=c.function.arguments.as_deref().unwrap_or("{}");
            let arguments=serde_json::from_str(raw).with_context(||format!("invalid JSON tool arguments for {name}"))?;
            calls.push(ToolCall{id,name,arguments});
        }
        let usage=envelope.usage.unwrap_or(UsageEnvelope{prompt_tokens:None,completion_tokens:None});
        Ok(CompletionResponse{text:choice.message.content.clone().unwrap_or_default(),tool_calls:calls,finish_reason:choice.finish_reason.clone(),usage:Usage{input_tokens:usage.prompt_tokens.unwrap_or(0).min(u32::MAX as u64) as u32,output_tokens:usage.completion_tokens.unwrap_or(0).min(u32::MAX as u64) as u32}})
    }
}
