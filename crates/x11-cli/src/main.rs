...
async fn main()->Result<()>{
    ...
    match Cli::parse().command {
        Some(Command::Run{goal,workspace,model,yes,max_iterations,session,verify_commands,hooks,verification_timeout_ms,mode})=>{
            let mut cfg=AgentConfig::default();
            cfg.workspace=workspace.unwrap_or(std::env::current_dir()?);
            cfg.mode=parse_mode(&mode)?;
            if let Some(m)=model{cfg.model=m;}
            cfg.auto_approve=yes || matches!(cfg.mode, AgentMode::Auto);
            ...
            if let (Ok(key),Ok(base))=(std::env::var("X11_API_KEY"),std::env::var("X11_BASE_URL")){
                let mut agent=AgentRuntime::new(goal,cfg,OpenAiCompatible::new(base,key));
                let result=agent.run().await; for e in &agent.session.events{println!("{:?}",e);} result.map(|text|{if !text.is_empty(){println!("\n{text}")}})?;
            } else {
                let mut agent=AgentRuntime::new(goal,cfg,MockProvider); let result=agent.run().await; for e in &agent.session.events{println!("{:?}",e);} result?;
            }
        }
        Some(Command::Sessions{command})=>{
            let workspace=match &command { SessionCommand::List{workspace}|SessionCommand::Show{workspace,..}|SessionCommand::Fork{workspace,..} => workspace.clone().unwrap_or(std::env::current_dir()?) };
            let store=store_for(workspace);
            match command {
                SessionCommand::List{..}=>{for (id,updated,goal) in store.list().await?{println!("{id}  {updated}  {goal}");}}
                SessionCommand::Show{id,..}=>{let s=store.load(id).await?;println!("{}\n{}\n{} events",s.id,s.goal,s.events.len());}
                SessionCommand::Fork{id,goal,..}=>{let s=store.load(id).await?;let fork=s.fork(goal);let path=store.save(&fork).await?;println!("forked {} -> {}\n{}",s.id,fork.id,path.display());}
            }
        }
        None=>println!("X11 Code. Use `x11 run <goal>` or `x11 sessions list`.")
    }
    Ok(())
}
