...
enum Command {
    ...
    Uninstall { #[arg(long)] purge_config: bool },
...
async fn run_with_provider<P:x11_model::ModelProvider+'static>(goal:String,cfg:AgentConfig,provider:P,tui:bool)->Result<()>{
 ...
 let plugin_dir=agent.config.workspace.join(".x11/plugins");if plugin_dir.is_dir(){let count=agent.load_plugins_from(&plugin_dir)?;if count>0{println!("loaded {count} plugin skill(s) from {}",plugin_dir.display());}}
 let mcp_path=std::env::var_os("X11_MCP_CONFIG").map(PathBuf::from).filter(|p|p.is_file()).or_else(||{let p=agent.config.workspace.join(".x11/mcp.json");p.is_file().then_some(p)});
 ...
 if tui{
  let approval_requests=agent.enable_interactive_approvals();let broker=agent.approvals.clone();let events=agent.subscribe();let cancel=agent.cancel.clone();let mut handle=tokio::spawn(async move{let mut agent=agent;agent.run().await});
...
async fn main()->Result<()>{
 ...
 match Cli::parse().command{
  ...
  Some(Command::Uninstall{purge_config})=>{uninstall::run(purge_config)?;},
  ...
  None=>println!("X11 Code. Use `x11 doctor`, `x11 update`, `x11 uninstall`, `x11 run <goal> [--tui]`, or `x11 sessions list`.")
 }
 Ok(())
}
