...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Backend { None, Bubblewrap, MacSeatbelt, WindowsRestrictedToken }
...
impl Capability {
    pub fn available(backend: Backend) -> bool { !matches!(backend, Backend::None) }
}
...
pub fn detect() -> Capability {
    ...
    if cfg!(target_os = "macos") {
        ...
        return Capability { backend: if available { Backend::MacSeatbelt } else { Backend::None }, filesystem_isolation: available, network_isolation: available, process_isolation: available, reason: if available { "macOS Seatbelt executable available" } else { "macOS sandbox backend unavailable" }.into() };
    }
    if cfg!(target_os = "windows") {
        return Capability { backend: Backend::WindowsRestrictedToken, filesystem_isolation: true, network_isolation: false, process_isolation: true, reason: "Windows restricted-token backend; fine-grained network isolation is not enabled yet".into() };
    }
...
pub fn wrap_command(mode: SandboxMode, workspace: &Path, program: &Path, args: &[String]) -> Result<(PathBuf, Vec<String>, Backend)> {
    ...
    match (mode, capability.backend) {
        ...
        (_, Backend::Bubblewrap) => {
            ...
            let mut wrapped = vec!["--die-with-parent".into(), "--new-session".into(), "--unshare-pid".into(), "--unshare-uts".into(), "--unshare-ipc".into(), "--unshare-net".into(), "--proc".into(), "/proc".into(), "--dev".into(), "/dev".into(), "--ro-bind".into(), "/usr".into(), "/usr".into(), "--ro-bind".into(), "/bin".into(), "/bin".into()];
