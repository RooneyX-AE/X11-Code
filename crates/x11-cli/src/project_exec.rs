...
pub fn detect(workspace:&Path)->ProjectKind{
 ...
 else if workspace.join("Cargo.toml").is_file(){ProjectKind::Rust}else{ProjectKind::Unknown}
}
...
fn node_tool(workspace:&Path,action:NodeAction)->Result<(PathBuf,Vec<String>,Option<RuntimeStatus>)>{
 ...
 let args=match(manager,action){
  ...
  (NodeManager::Npm,NodeAction::Build)|(NodeManager::Pnpm,NodeAction::Build)|(NodeManager::Yarn,NodeAction::Build)=>vec!["run","build"],
 };
...
fn python_tool(workspace:&Path,args:Vec<&str>)->Result<(PathBuf,Vec<String>,Option<RuntimeStatus>)>{
 let status=runtime_status(workspace,RuntimeKind::Python)?;anyhow::ensure!(status.source!=Source::Missing,"Python runtime is missing; run `x11 runtime install python <version>`");
 let python=status.executable.clone().context("resolved Python executable missing")?;Ok((python,args.into_iter().map(str::to_owned).collect(),Some(status)))
}
...
fn find_program(name:&str)->Result<PathBuf>{let path=env::var_os("PATH").context("PATH is unavailable")?;for root in env::split_paths(&path){let c=root.join(name);if c.is_file(){return Ok(c)}if cfg!(windows){for s in [".exe",".cmd"]{let c=root.join(format!("{name}{s}"));if c.is_file(){return Ok(c)}}}}anyhow::bail!("required executable '{name}' is not available on PATH")}
...
#[cfg(test)]
mod tests{use super::*;
 #[test]fn detects_common_projects(){let root=std::env::temp_dir().join(format!("x11-exec-{}",std::process::id()));std::fs::create_dir_all(&root).unwrap();std::fs::write(root.join("package.json"),"{}").unwrap();assert_eq!(detect(&root),ProjectKind::Node);std::fs::remove_dir_all(root).unwrap();}
 #[test]fn package_manager_field_has_priority(){let root=std::env::temp_dir().join(format!("x11-pm-{}",std::process::id()));std::fs::create_dir_all(&root).unwrap();std::fs::write(root.join("package.json"),r#"{"packageManager":"pnpm@10.0.0"}"#).unwrap();std::fs::write(root.join("package-lock.json"),"{}").unwrap();assert_eq!(detect_node_manager(&root).unwrap(),NodeManager::Pnpm);std::fs::remove_dir_all(root).unwrap();}
 #[test]fn lockfiles_select_install_policy(){let root=std::env::temp_dir().join(format!("x11-lock-{}",std::process::id()));std::fs::create_dir_all(&root).unwrap();std::fs::write(root.join("package.json"),"{}").unwrap();std::fs::write(root.join("pnpm-lock.yaml"),"lockfileVersion: '9'\n").unwrap();assert_eq!(detect_node_manager(&root).unwrap(),NodeManager::Pnpm);std::fs::remove_dir_all(root).unwrap();}
 #[test]fn unknown_project_is_rejected(){assert!(plan(Path::new("."),ProjectKind::Unknown,"test").is_err())}
 #[test]fn runtime_path_uses_executable_parent(){let status=RuntimeStatus{kind:RuntimeKind::Node,source:Source::System,executable:Some(PathBuf::from("/opt/node/bin/node")),version:Some("22.0.0".into()),requested:None,project_reason:None};assert_eq!(runtime_path(&status),Some(PathBuf::from("/opt/node/bin")));}
}