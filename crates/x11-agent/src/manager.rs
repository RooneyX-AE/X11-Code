 ...
 async fn run_batch(&self,specs:Vec<SubagentSpec>)->Result<Vec<SubagentResult>>{
  ...
  for spec in specs{
   ...
   jobs.spawn(async move{
    let _permit=permit;cfg.workspace=workspace;cfg.max_iterations=spec.max_iterations.max(1);cfg.model=spec.model.clone();cfg.max_context_tokens=(spec.token_budget as usize).clamp(2_000,128_000);cfg.auto_approve=false;cfg.session_path=None;
    ...
    let goal=format!("Role: {role_prompt}\nTask: {}\nTool budget: {} calls. Preferred tools: {}",spec.goal,spec.tool_budget,if spec.allowed_tools.is_empty(){"all registered tools".into()}else{spec.allowed_tools.iter().cloned().collect::<Vec<_>>().join(", ")});let mut runtime=AgentRuntime::new_shared(goal,cfg,provider);let session_id=runtime.snapshot.session_id;
...
#[cfg(test)]
mod tests{use super::*;use x11_model::MockProvider;use std::collections::BTreeSet;
