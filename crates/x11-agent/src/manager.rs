...
impl<P: ModelProvider + 'static> AgentManager<P> {
    pub fn new(
        ...
        Self {
            ...
            base_config,
            config,
            locks: WorkspaceLockManager::default(),
        }
    }
    ...
    fn swarm_id(&self, fallback: Uuid) -> Uuid {
        self.config.swarm_id.unwrap_or(fallback)
    }
    ...
    fn resolve_workspace_scope(&self, scope: Option<&str>) -> Result<PathBuf> {
        let root = std::fs::canonicalize(&self.workspace)
            .map_err(|e| anyhow!("failed to canonicalize workspace {}: {e}", self.workspace.display()))?;
        let Some(scope) = scope else { return Ok(root); };
        let rel = Path::new(scope);
        if rel.is_absolute() || rel.components().any(|c| matches!(c, std::path::Component::ParentDir)) {
            anyhow::bail!("workspace_scope must be relative and cannot contain '..'");
        }
        let candidate = root.join(rel);
        let canonical = std::fs::canonicalize(&candidate)
            .map_err(|e| anyhow!("workspace_scope does not exist or cannot be resolved: {} ({e})", candidate.display()))?;
        if !canonical.starts_with(&root) || canonical == root {
            anyhow::bail!("workspace_scope escapes the parent workspace: {}", scope);
        }
        Ok(canonical)
    }
    ...
    async fn run_batch(&self, specs: Vec<SubagentSpec>) -> Result<Vec<SubagentResult>> {
        ...
        for spec in specs {
            ...
            jobs.spawn(async move {
                ...
                if let Some(bus) = &bus {
                    bus.emit(
                        SwarmEvent::new(swarm_id, SwarmEventKind::TaskStarted)
                            .task(spec.id.clone())
                            .state("running")
                            .progress(1),
                    );
                    ...
                    match &spec.workspace_scope {
                        ...
                        Some(scope) => {
                            ...
                            let canonical = std::fs::canonicalize(&candidate).map_err(|e| {
                                anyhow!("workspace_scope does not exist for {}: {} ({e})", spec.id, candidate.display())
                            })?;
                            if !canonical.starts_with(&root) || canonical == root {
                ...
                let preferred_tools = if spec.allowed_tools.is_empty() {
                    "all registered tools".to_string()
                } else {
                    spec.allowed_tools.iter().cloned().collect::<Vec<_>>().join(", ")
                };
                let goal = format!(
                    "Role: {role_prompt}\nTask: {}\nTool budget: {} calls. Preferred tools: {preferred_tools}",
                    spec.goal, spec.tool_budget
                );
                ...
                runtime.executor = if spec.allowed_tools.is_empty() {
                    ...
                    ToolExecutor::with_locks_and_allowlist(
                        runtime.tools.clone(),
                ...
                let files_changed = runtime
                    .session
                    .events
                    .iter()
                    .filter_map(|event| match event {
                        AgentEvent::ToolRequested { tool, input, .. }
                            if tool == "write_file" || tool == "edit_file" => input["path"].as_str().map(str::to_owned),
                        _ => None,
                    })
                    .collect::<BTreeSet<_>>()
                    .into_iter()
                ...
                if let Some(bus) = &bus {
                    bus.emit(
                        SwarmEvent::new(swarm_id, SwarmEventKind::VerificationStarted)
                            .task(spec.id.clone())
                            .agent(spec.id.clone())
                            .state("verifying"),
                    );
                    let verification_kind = if cancelled {
                    ...
                    bus.emit(
                        SwarmEvent::new(swarm_id, verification_kind)
                            .task(spec.id.clone())
                            .agent(spec.id.clone())
                            .progress(100)
                            .state(if cancelled { "cancelled" } else if success { "passed" } else { "failed" })
                            .evidence(verification.clone()),
                    );
                    bus.emit(
                        SwarmEvent::new(
                            swarm_id,
                            if success { SwarmEventKind::TaskCompleted } else if cancelled { SwarmEventKind::TaskCancelled } else { SwarmEventKind::TaskFailed },
                        )
                        .task(spec.id.clone())
                        .agent(spec.id.clone())
                        .progress(100)
                        .state(if cancelled { "cancelled" } else if success { "completed" } else { "failed" })
                        .evidence(format!("files_changed={}", files_changed.len())),
                    );
                }
        ...
        let mut results = Vec::with_capacity(semaphore.available_permits().max(1));
    ...
    pub async fn run_parallel(&self, specs: Vec<SubagentSpec>) -> Result<Vec<SubagentResult>> {
        Ok(self.run_report(specs).await?.results)
    }

    pub async fn run_report(&self, specs: Vec<SubagentSpec>) -> Result<SwarmReport> {
        self.run_report_internal(specs, self.config.state_path.clone()).await
    }

    pub async fn run_report_resumable(
        &self,
        specs: Vec<SubagentSpec>,
        state_path: impl Into<PathBuf>,
    ) -> Result<SwarmReport> {
        self.run_report_internal(specs, Some(state_path.into())).await
    }

    async fn run_report_internal(
        &self,
        specs: Vec<SubagentSpec>,
        ...
        let mut state = if let Some(path) = &state_path {
            match SwarmState::load(path).await {
                Ok(mut loaded) => {
                    ...
                    self.emit(
                        SwarmEvent::new(loaded.swarm_id, SwarmEventKind::SwarmResumed)
                            .state("resumed")
                            .progress(0),
                    );
                    loaded
            ...
            SwarmState::new("swarm", task_ids.clone())
        };
        let swarm_id = self.swarm_id(state.swarm_id);
        state.swarm_id = swarm_id;

        self.emit(
            SwarmEvent::new(swarm_id, SwarmEventKind::SwarmStarted)
                .state("running")
                .progress(0),
        );
        ...
        for id in state.tasks.keys() {
            if !known.contains(id.as_str()) {
                anyhow::bail!("persisted swarm task '{id}' is absent from supplied specs");
            }
        }
        ...
        let mut completed = state
            .tasks
            .iter()
            .filter(|(_, task)| matches!(task.status, SwarmTaskStatus::Succeeded))
            .map(|(id, _)| id.clone())
            .collect::<BTreeSet<_>>();
        let mut pending = specs
            .into_iter()
            .filter(|spec| !completed.contains(&spec.id))
        ...
        while !pending.is_empty() {
                ...
                for id in pending.keys() {
                    let _ = state.mark_finished(
                        ...
                        ResultSnapshot {
                            success: false,
                            cancelled: true,
                            output: Some("cancelled before scheduling".into()),
                            error: Some("parent swarm cancelled".into()),
            ...
            let mut ready = pending
                .values()
                .filter(|spec| spec.dependencies.iter().all(|dependency| completed.contains(dependency)))
                .cloned()
                .collect::<Vec<_>>();
                ...
                for id in pending.keys() {
                    let _ = state.mark_finished(
                        ...
                        ResultSnapshot {
                            ...
                            cancelled: false,
                            output: None,
                            error: Some(format!("blocked by unresolved dependency graph: {unresolved}")),
                            files_changed: Vec::new(),
                        },
                    );
                    self.emit(
                        SwarmEvent::new(swarm_id, SwarmEventKind::TaskBlocked)
                            .task(id.clone())
                            .state("blocked")
            ...
            for spec in &ready {
                self.emit(
                    SwarmEvent::new(swarm_id, SwarmEventKind::TaskQueued)
                        .task(spec.id.clone())
                        .parent_task("root")
                        .state("queued")
                        .progress(0),
                ...
                if let Some(path) = &state_path {
                    state.mark_running(&spec.id, Uuid::nil())?;
        ...
        let failed = results.len().saturating_sub(succeeded)
            + state.tasks.values().filter(|task| matches!(task.status, SwarmTaskStatus::Failed | SwarmTaskStatus::Cancelled)).count();

        let mut paths: BTreeMap<String, Vec<String>> = BTreeMap::new();
        for result in &results {
            for path in &result.files_changed {
                paths.entry(path.clone()).or_default().push(result.id.clone());
            }
        }
        let conflict_candidates = paths
            .into_iter()
            .filter_map(|(path, mut owners)| {
                owners.sort();
                owners.dedup();
                (owners.len() > 1).then(|| format!("{path} <- {}", owners.join(", ")))
            })
            .collect::<Vec<_>>();

        for conflict in &conflict_candidates {
            self.emit(SwarmEvent::new(swarm_id, SwarmEventKind::ConflictDetected).state("conflict").evidence(conflict.clone()));
        }

        self.emit(
            SwarmEvent::new(swarm_id, SwarmEventKind::SwarmCompleted)
                .state(if failed == 0 { "completed" } else { "completed_with_failures" })
                .progress(100)
                .evidence(format!("succeeded={succeeded},failed={failed}")),
        );
...
mod tests {
    ...
    async fn manager_runs_isolated_agents_in_parallel() {
        let manager = AgentManager::new(
            ...
            PathBuf::from("."),
            AgentConfig::default(),
            AgentManagerConfig { max_concurrency: 2, ..Default::default() },
        );
        let results = manager
            .run_parallel(vec![
                spec("b", SubagentRole::Explorer, 0, &[]),
                spec("a", SubagentRole::Tester, 0, &[]),
            ])
            .await
            .unwrap();
    ...
    async fn dependencies_are_scheduled_in_batches() {
        ...
        let report = manager
            .run_report(vec![
                spec("review", SubagentRole::Reviewer, 0, &["impl"]),
                spec("impl", SubagentRole::Implementer, 1, &[]),
            ])
            .await
            .unwrap();
    ...
    async fn missing_dependency_is_reported_as_failure() {
        ...
        let report = manager
            .run_report(vec![spec("a", SubagentRole::Explorer, 0, &["missing"])])
            .await
            .unwrap();
    ...
    async fn persisted_successes_are_skipped_on_resume() {
        ...
        let report = manager
            .run_report_resumable(
                vec![
                    spec("a", SubagentRole::Explorer, 0, &[]),
                    spec("b", SubagentRole::Explorer, 0, &[]),
                ],
                &path,
            )
    ...
    async fn inherited_cancellation_marks_children_cancelled() {
        ...
        let manager = AgentManager::new(
            ...
            PathBuf::from("."),
            AgentConfig::default(),
            AgentManagerConfig { cancellation: Some(tx), ..Default::default() },
        );
        let report = manager
            .run_report(vec![spec("a", SubagentRole::Explorer, 0, &[])])
            .await
            .unwrap();
    ...
    async fn emits_lifecycle_events() {
        ...
        let manager = AgentManager::new(
            ...
            PathBuf::from("."),
            AgentConfig::default(),
            AgentManagerConfig { event_bus: Some(bus.clone()), swarm_id: Some(id), ..Default::default() },
        );
