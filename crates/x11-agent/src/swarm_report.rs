...
impl SwarmReportSnapshot {
    pub fn to_pretty_json(&self) -> serde_json::Result<String> { serde_json::to_string_pretty(self) }
}
...
mod tests {
    ...
    fn snapshot_round_trips_as_json() {
        let report = SwarmReport { results: Vec::new(), succeeded: 0, failed: 0, conflict_candidates: vec!["src/lib.rs <- a, b".into()] };
